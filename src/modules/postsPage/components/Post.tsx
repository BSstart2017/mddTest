...
export const Post: FC<PropsType> = ({post, comments}) => {
  ...
  const renderComments = useMemo(() => (postId: number) => {
    const visibleComments: CommentType[] = showAllComments[postId] ? comments : comments.length > 0 ? [comments[0]] : [];
  ...
  return (
    <ListGroup.Item >
      ...
      <Button onClick={() => toggleComments(post.id)}>
        {(showAllComments[post.id] ? 'hide' : 'show') + ' comments ' + comments.length}
      </Button>
